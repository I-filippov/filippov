    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FIO=model.FIO, PhoneNumber=model.PhoneNumber,Date_start=model.Date_start};
                user.Photo = imageData;
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    await UserManager.AddToRoleAsync(user.Id, "custumer");
        ...
        public ActionResult AccountSettings(bool? savedSuccesfully)
        {
            ...
            AccountSettingsViewModel model = new AccountSettingsViewModel { FIO = user.FIO, Email = user.Email, PhoneNumber = user.PhoneNumber, Date_start = user.Date_start, Adres = user.Adres, Pasport = user.Pasport, SNILS = user.SNILS, INN = user.INN, Job_status = user.Job_status, Filial = user.Filial, Salary = user.Salary, Photo = user.Photo };
        ...
        public async Task<ActionResult> AccountSettings(AccountSettingsViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user.Pasport = model.Pasport;
                user.SNILS = model.SNILS;
                user.INN = model.INN;
                user.Job_status = model.Job_status;
                user.Filial = model.Filial;
